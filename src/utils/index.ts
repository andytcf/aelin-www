...
import WhitelistedStakingRewards from "constants/abi/WhitelistedStakingRewards.json";
...
export const getPoolContract = (provider: provider, address: string) => {
  const web3 = new Web3(provider);
  const contract = new web3.eth.Contract((WhitelistedStakingRewards.abi as unknown) as AbiItem, address);
  return contract;
};
