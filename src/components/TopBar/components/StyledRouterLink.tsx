  ...
  if (children) {
    return (
      <div
        ...
        style={
          mobileMenu
            ? { cursor: "pointer", width: "100%", textAlign: "left", display: "flex", flexDirection: "column" }
            : { cursor: "pointer", width: "100%", textAlign: "left" }
        }
      >
        <StyledLink subactive={isActive} exact activeClassName="active" style={style} to={target || ""}>
          {label}
          {mobileMenu && <StyledSpan>{isShow === true ? "➖" : "➕"}</StyledSpan>}
        </StyledLink>
        {isShow === true && <StyledNestedMenu>{children}</StyledNestedMenu>}
      </div>
    );
  } else {
    return (
      <StyledLink exact activeClassName="active" to={target || ""} style={style} onClick={onDismiss}>
        {label}
      </StyledLink>
    );
  }
